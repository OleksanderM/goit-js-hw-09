...
const options = {
    ...
    onClose (selectedDates) {
        if (selectedDates[0].getTime() > todaysDate.getTime()) return refs.start.disabled = false;
    ...
    onChange (selectedDates) {
        localStorage.setItem("date", selectedDates[0].getTime() - todaysDate.getTime());
    }
};
...
const render = ()=>{
    if (localStorage.getItem("date") < 1000) {
        (0, _notiflixDefault.default).Confirm.show("Woohoooo!!", "The time has come, my friend!", "Let's go!", "I'm ready!", {
            position: "center-center"
        });
